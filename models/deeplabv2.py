...
class CrossNorm_list(nn.Module):
    ...
    def __init__(self, nb_target):
        ...
        self.mean_list = [torch.tensor(0.0).cuda() for i in range(nb_target)]
        self.std_list = [torch.tensor(1.0).cuda() for i in range(nb_target)]

        self.current_id, self.transfer_id = 0, 0
    def forward(self, x):
        size = x.size()
        ...
        x = normalized_feat * self.std_list[self.transfer_id].expand(size) + self.mean_list[self.transfer_id].expand(size)

        # # update mean and var
        # self.mean_list[self.current_id] = content_mean.detach()
        # self.std_list[self.current_id] = content_std.detach()

        return x

    def update_ins_mean_std(self, x):
        with torch.no_grad():
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.cross:
            out = self.real_cn(out, self.current_id, self.transfer_id)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.update:
            self.real_cn.update_ins_mean_std(out)
...
class ResNet101(nn.Module):
    def __init__(self, block, layers, num_classes, BatchNorm, num_target=1, bn_clr=False):
        self.inplanes = 64
        self.bn_clr = bn_clr
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                #            i.requires_grad = False
            elif isinstance(m, CrossNorm_list):
                self.cn_modules.append(m)

            
        # crossnorm on first layer
        for m in self.layer1.modules():
            if isinstance(m, Bottleneck):
                self.bottleneck_modules.append(m)

        # crossnorm on second layer
    ...
    def update_id(self, current_id, transfer_id):
        for i in range(len(self.cn_modules)):
            self.cn_modules[i].current_id = current_id
            self.cn_modules[i].transfer_id = transfer_id

    def _enable_cross_norm(self):
        for m in self.bottleneck_modules:
            m.cross = True
    ...
    def forward(self, x, domain_list, ssl=False, lbl=None):
        _, _, h, w = x.size()
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
def Deeplab(BatchNorm, num_classes=7, num_target=1, freeze_bn=False, restore_from=None, initialization=None, bn_clr=False):
